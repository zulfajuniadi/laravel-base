...
class ModelNamesMenu extends BaseMenu
{
    ...
    public function show($params)
    {
        $this->menu->handler('breadcrumbs')
            ->addItem(action('ModelNamesController@index'), trans('model-names.model_names'))
            ->addItem(action('ModelNamesController@show', $params['model_names']->slug), $params['model_names']->name);
        ...
        $this->menu->handler('model-names.record-buttons.show')
            ->addItemIf($this->check('edit', $params), action('ModelNamesController@edit', $params['model_names']->slug), trans('model-names.edit'), 'btn btn-primary')
            ->addItemIf($this->check('revisions', $params), action('ModelNamesController@revisions', $params['model_names']->slug), trans('model-names.revisions'), 'btn btn-default')
            ->addItemIf($this->check('duplicate', $params), action('ModelNamesController@duplicate', $params['model_names']->slug),  trans('model-names.duplicate'), 'btn btn-default')
            ->addItemIf($this->check('delete', $params), action('ModelNamesController@delete', $params['model_names']->slug), trans('model-names.delete'), 'btn btn-danger confirm-action');
    }
    ...
    public function revisions($params)
    {
        $this->menu->handler('breadcrumbs')
            ->addItem(action('ModelNamesController@index'), trans('model-names.model_names'))
            ->addItem(action('ModelNamesController@show', $params['model_names']->slug), $params['model_names']->name)
            ->addItem(action('ModelNamesController@revisions', $params['model_names']->slug), trans('model-names.revisions'));
